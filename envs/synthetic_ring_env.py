...
class sumo_env_ring():
	...
	def start(self, gui=False, warm_up_step=1000,network_conf="networks/ring_network/sumoconfig.sumo.cfg", network_xml='networks/ring_network/highway.net.xml'):
		self.gui = gui
	...
	def step(self, action,veh_id_list,max_dec=-3,max_acc=3,stop_and_go=False,sumo_lc=False,sumo_carfollow=False,lane_change='SECRM',car_follow='Gipps'):
		...
		next_state=[]
		for i in range(len(veh_id_list)):
			next_state = self.get_ego_veh_info(veh_id_list[i]) ##length is 65
	...
	def reset(self, gui=False):
		...
		for i in range(len(veh_id_list)):
			self.rl_vehicles_state[veh_id_list[i]]=self.get_ego_veh_info(veh_id_list[i])
